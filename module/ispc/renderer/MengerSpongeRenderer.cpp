...
void MengerSpongeRenderer::commit()
{
    ...
    _farPlane = getParam1f("farPlane", 100.f);
    _mapIterations = getParam1i("mapIterations", 4);
    float timestamp = getParam1f("timestamp", 0.f);

    ispc::MengerSpongeRenderer_set(getIE(), (ispc::vec3f&)_bgColor, _shadows,
                                   _softShadows, _lightPtr, _lightArray.size(),
                                   _samplesPerRay, _farPlane, _mapIterations,
                                   timestamp);
}
