...
# Docker container for running the fractals as a plugin within Brayns as a service
...
# Image where Brayns+fractals plugin is built
...
RUN rm -rf ${FRACTALS_SRC}/build \
   && mkdir -p ${FRACTALS_SRC}/build \
   && cd ${FRACTALS_SRC}/build \
   && PATH=${ISPC_PATH}/bin:${PATH} CMAKE_PREFIX_PATH=${DIST_PATH} cmake .. \
   -DCMAKE_INSTALL_PREFIX=${DIST_PATH}


RUN cd ${FRACTALS_SRC}/build && make -j install VERBOSE=1

# Final image, containing only Brayns and fractals and libraries required to run it
...
# When running `docker run -ti --rm -p 8200:8200 fractals`,
...
CMD ["--http-server", ":8200", "--plugin", "braynsFractals"]
